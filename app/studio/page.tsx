...
export default function StudioPage(){
  ...
  const [schemaText, setSchemaText] = React.useState<string>(() => initial || JSON.stringify({ blocks: [{type:"text", props:{text:"Hello Studio!"}}] }, null, 2));
            ...
            <div className="mb-3 flex items-center justify-between">
              ...
              <div className="flex gap-2 text-sm">
                {["mobile","tablet","full"].map((d) => (
                  <button key={d} onClick={()=>setDevice(d as any)} className={"h-8 px-3 rounded-md border " + (device===d ? "bg-card" : "")}>{d}</button>
                ))}
              </div>
            </div>
            <div className={clsx("border rounded-md bg-white mx-auto", {
