export function Stepper() {
  const steps = ["Market analysis","Visualization","Legal review","Assembly","Launch","Support"];
  return (
    <ol className="grid gap-4 md:grid-cols-3 lg:grid-cols-6">
      {steps.map((s, i) => (
        <li key={s} className="relative rounded-xl border bg-card p-4">
          <span className="absolute -top-3 left-4 inline-flex h-6 items-center rounded-full bg-[--primary] px-2 text-xs text-[--primary-foreground]">Step {i+1}</span>
          <h3 className="mt-2 font-medium">{s}</h3>
          <p className="mt-1 text-sm text-muted-foreground">Short description of the step outcome.</p>
        </li>
      ))}
    </ol>
  );
}
